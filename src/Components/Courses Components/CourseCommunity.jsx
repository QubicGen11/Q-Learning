...
const CourseCommunity = ({ courseId }) => {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      // If no comments, set empty array without showing error
      ...
      const commentsWithData = await Promise.all(commentsResponse.data.map(async (comment) => {
        try {
          ...
          // Silently handle individual comment data fetch errors
          return {
            ...comment,
            likes: 0,
            replies: [],
      ...
      // Only show error toast for network/server errors, not for empty comments
